#!/usr/bin/env python3
"""
Bridge Monitor, live feed of agent Claude delegations.
...
LOG_FILE = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.path.dirname(os.path.abspath(__file__)), "bridge.log")
...
def print_header():
    print("\n" * 2)
    print(BOLD + CYAN + "  Bridge Monitor ,  Claude delegation feed" + RESET + DIM + "  (live)" + RESET)
    print(divider("═", CYAN))
...
def render_start(ev, pending):
    ...
    print(BOLD + YELLOW + f"  TASK" + RESET + DIM + f"  •  {fmt_ts(ev.get('ts',''))}" + RESET)
    print(divider("─"))
    print()
...
def render_done(ev, pending):
    ...
    print(GREEN + BOLD + f"  RESPONSE" + RESET + DIM + f"  •  {fmt_ts(ev.get('ts',''))}  •  {ev.get('duration','?')}s  •  ${ev.get('cost_usd',0):.4f}" + RESET)
    print(divider("─"))
    print()
...
def render_error(ev, pending):
    pending.pop(ev.get("id","?"), None)
    print(RED + BOLD + f"  ERROR" + RESET + DIM + f"  •  {fmt_ts(ev.get('ts',''))}  •  {ev.get('duration','?')}s" + RESET)
    print(divider("─"))
...
def render_timeout(ev, pending):
    pending.pop(ev.get("id","?"), None)
    print(RED + BOLD + f"  TIMEOUT" + RESET + DIM + f"  •  {fmt_ts(ev.get('ts',''))}  •  {ev.get('duration','?')}s" + RESET)
    print(divider("─"))
    print()
    sys.stdout.flush()
